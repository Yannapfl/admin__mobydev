      ...
      <div className="cards-group">
        {data.categories.map((category) => (
          <ContentCard
            ...
            onEdit={canEdit(editKey) ? () => openModal('text', { mode: 'edit', entity: category, id: category.categoryId }) : null}
            onDelete={canEdit(editKey) ? () => openModal('delete', { 
              label: 'категорию', 
              onConfirm: () => {
                deleteEntity('categories', category.categoryId, 'category');
                closeModal();
              } 
            }) : null}
          />
        ...
        <ModalFactory
          ...
          modalProps={{
            ...
            onAdd: canEdit(editKey) ? (newCategory) => {
              addEntity('categories', newCategory, 'category');
              closeModal();
            } : null,
            onUpdate: canEdit(editKey) ? (updatedCategory) => {
              updateEntity('categories', updatedCategory, 'category');
              closeModal();
            } : null,
            onDelete: canEdit(editKey) ? () => {
              deleteEntity('categories', modalProps.entity?.categoryId, 'category');
              closeModal();
            } : null,
            closeModal,
          }}
